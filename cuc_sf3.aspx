								...
								if(confirm("確認要完工?")){
									$('#cucs .cucs_waste').each(function(index) {
										if($(this).prop('checked')){
											...
											q_func('qtxt.query.enda', 'cuc_sf.txt,waste,'+r_accy+';'+$('#cucs_noa'+n).text()+';'+$('#cucs_noq'+n).text()+';'+r_userno.toUpperCase()+';'+r_name);
										}
									});
								}else{
									t_mins_count=0;
                    ...
                    case 'tocub':
                    	...
                    	if (as[0] != undefined){
                    		...
                    		if(t_err.length>0){
	                    		...
	                    		+r_userno.toUpperCase()+';'+r_name+';'+t_noa+';'+t_noq+';'+t_xmount+';'+t_xcount+';'+t_xweight+';2');
	                    		//取消刷新
	                    		//clearInterval(intervalupdate);
							}
                    	}else{
                ...
                if(t_func.indexOf('qtxt.query.cuctcheckuno_')>-1){
                	...
                	if(getunocount==0){ //批號檢查完畢
                		if(getunoerr.length>0){
							...
							if(t_nouno.length>0){
								q_func('qtxt.query.cubnouno', 'cuc_sf.txt,cubnouno_sf,' + encodeURI(r_accy) + ';' + encodeURI(t_nouno)+ ';' + encodeURI(t_mechno)+ ';' + encodeURI(r_userno.toUpperCase())+ ';' + encodeURI(r_name)+';2');
								Lock();
							}
                		}
                	}
