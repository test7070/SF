...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr">
	<head>
		...
		<script type="text/javascript">
            ...
            function mainPost() {
                q_mask(bbmMask);
                $('#txtProduct').change(function(e) {
                    if ($(this).val().length > 0) {
                        t_where = "where=^^ product='" + $(this).val() + "'^^";
                        q_gt('adpro', t_where, 0, 0, 0, "checkProduct_change", r_accy);
                    }
                });

                $('#vewProduct').text("續接器");
                $('#lblProduct').text("續接器");
                document.title = '續接器主檔';
            }
            ...
            function q_gtPost(t_name) {
                switch (t_name) {
                    case 'checkProduct_change':
                        var as = _q_appendData("adpro", "", true);
                        if (as[0] != undefined) {
                            alert('已存在 ' + as[0].product);
                        }
                        break;
                    case 'checkProductno_btnOk':
                        var as = _q_appendData("adpro", "", true);
                        if (as[0] != undefined) {
                            alert('已存在 ' + as[0].product);
                            return;
            ...
            function btnOk() {
                ...
                if (q_cur == 1) {
                    t_where = "where=^^ product='" + t_product + "'^^";
                    q_gt('adpro', t_where, 0, 0, 0, "checkProductno_btnOk", r_accy);
                } else {
                    wrServer(t_noa);
                }
            }
			...
			<div class="dview" id="dview" style="float: left;  width:40%;"  >
				<table class="tview" id="tview"   border="1" cellpadding='2'  cellspacing='0' style="background-color: #FFFF66;">
					<tr>
						...
						<td align="center" style="width:50%"><a id='vewProduct'> </a></td>
					</tr>
					<tr>
